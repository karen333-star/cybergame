...
function cambiarValor(id, delta) {
    ...
    // Mantener el estado global sincronizado para toda la aplicación
    if (typeof window.estadoPartida === 'object') {
        if (id === 'cia' || id === 'presupuesto' || id === 'despido' || id === 'maxRondas') {
            window.estadoPartida[id] = valor;
        }
    }

    // Actualiza la UI visual de la sección
    actualizarConfigVisual();
}
...
function actualizarConfigVisual() {
    const configs = [
        { id: 'cia', spanId: 'ciaValue', barsId: 'ciaBars', min: 30, max: 80 },
        { id: 'presupuesto', spanId: 'presupuestoValue', barsId: 'presupuestoBars', min: 10, max: 80 },
        { id: 'despido', spanId: 'despidoValue', barsId: 'despidoBars', min: 5, max: 80 },
        { id: 'maxRondas', spanId: 'maxRondasValue', barsId: 'maxRondasBars', min: 15, max: 40 }
    ];

    configs.forEach(function(config) {
        const input = document.getElementById(config.id);
        const span = document.getElementById(config.spanId);
        const bars = document.getElementById(config.barsId);
        const card = document.getElementById('card-' + config.id);
        if (!input || !span || !bars) return;

        const value = Number(input.value) || config.min;
        span.textContent = value;

        // Sincronizar con el estado global para evitar desincronizaciones
        if (window.estadoPartida && typeof window.estadoPartida === 'object') {
            window.estadoPartida[config.id] = value;
        }

        const range = config.max - config.min;
        let ratio = 0;
        if (range > 0) {
            ratio = (value - config.min) / range;
            if (ratio < 0) ratio = 0;
            if (ratio > 1) ratio = 1;
        }

        const activeCount = Math.round(ratio * 9);
        ...
        if (card) {
            const isDanger = config.id === 'despido';
            const valueContainer = document.getElementById(config.id + 'ValueContainer') || span.parentElement;
            
            if (isDanger) {
                if (value > 60) {
                    card.classList.add('danger');
                    if (valueContainer) valueContainer.classList.add('danger');
                } else {
                    card.classList.remove('danger');
                    if (valueContainer) valueContainer.classList.remove('danger');
                }
            }
        }
    });
